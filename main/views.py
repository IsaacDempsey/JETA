...
def get_start(request):
    if request.is_ajax():
        ...
        df = df.groupby(['stopid', 'address'], as_index=False).apply(lambda x: x[['lat','lng']].values.tolist()[0]).reset_index()
        df = df.rename(columns={0:'coord', 'stopid':'stop_id', 'stop_name':'stop_name'})

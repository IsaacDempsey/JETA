  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      for (var i = 0; i < uniqueRoutes.length; i++) {
        ...
        var row_id = 'row'+line_num;
        $(
          '<div class="col-sm-12 col-centered mobile-col-centered pt-1"><div id="'+row_id+'"data-toggle="collapse" href="#' +
            collapse_id +
            '" role="button" aria-expanded="false" aria-controls="' +
            collapse_id +
            '" style="color: white" id="open-tt" onclick="getSchedule(\'' +
            line_num +
            '\')"><div class="row p-2" id="tt"><div class="col-sm-6" id="tt-line">' +
            line_num +
            '</div><div class="col-sm text-right" id="tt-caret"><i class="fas fa-caret-down"></i></div></div></div><div class="collapse dayofservice" id="' +
            collapse_id +
            '"><a class="show-tt" onclick="currentTimeTable(3)"><div class="row p-2"><div class="col-sm">Weekdays</div><div class="col-sm text-right"><i class="fas fa-eye"></i></div></div></a><a class="show-tt" onclick="currentTimeTable(1)"><div class="row p-2"><div class="col-sm">Sunday </div><div class="col-sm text-right"> <i class="fas fa-eye"></i></div></div></a><a class="show-tt" onclick="currentTimeTable(2)"><div class="row p-2"><div class="col-sm">Saturday</div><div class="col-sm text-right"><i class="fas fa-eye"></i></div></div></a></div></div>'
        ).appendTo("#timetable-content");
      }
    }
  });
}
  ...
  $.each(schedule_data, function(i, el) {
    ...
    if (el.dayofservice == "Saturday") {
      ...
      $(
        '<div class="card-header" id="heading' +
          heading +
          '"><h5 class="mb-0"><button id="saturday-destination-name" class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseSaturday' +
          destination +
          '" aria-expanded="true" aria-controls="#collapseSaturday' +
          destination +
          '">Towards: ' +
          destination +
          '</button></h5></div><div id="collapseSaturday' +
          destination +
          '" class="collapse show" aria-labelledby="' +
          heading +
          '" data-parent="#accordionExample"><div class="card-body schedule-time-saturday" id="' +
          id_for_table +
          '"></div></div>'
      ).appendTo("#card-saturday");
      ...
      for (var i = 0; i < schedule_time.length; i = i + 4) {
        ...
        $(
          '<div class="row p-3 mp-5 schdule-rows"><div class="col-xs-3 col-sm-3">' +
            scheduletime1 +
    ...
    if (el.dayofservice == "Sunday") {
      ...
      $(
        '<div class="card-header" id="heading' +
          heading +
          '"><h5 class="mb-0"><button id="sunday-destination-name" class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseSunday' +
          destination +
          '" aria-expanded="true" aria-controls="collapseSunday' +
          destination +
          '">Towards: ' +
          destination +
          '</button></h5></div><div id="collapseSunday' +
          destination +
          '" class="collapse show" aria-labelledby="heading' +
          heading +
          '" data-parent="#accordionExample"><div class="card-body schedule-time-sunday" id="' +
          id_for_table +
          '"></div></div>'
      ).appendTo("#card-sunday");
      // Do something in Saturday DOM
      for (var i = 0; i < schedule_time.length; i = i + 4) {
        var scheduletime1 = schedule_time[i];
        var scheduletime2 = schedule_time[i + 1];
    ...
    if (el.dayofservice == "Weekday") {
      ...
      $(
        '<div class="card-header" id="heading' +
          heading +
          '"><h5 class="mb-0"><button id="weekday-destination-name" class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseWeekday' +
          destination +
          '" aria-expanded="true" aria-controls="collapseWeekday' +
          destination +
          '">Towards: ' +
          destination +
          '</button></h5></div><div id="collapseWeekday' +
          destination +
          '" class="collapse show" aria-labelledby="' +
          heading +
          '" data-parent="#accordionExample"><div class="card-body schedule-time-weekdays" id="' +
          id_for_table +
          '"></div></div>'
      ).appendTo("#card-weekdays");
      // Do something in Saturday DOM
      for (var i = 0; i < schedule_time.length; i = i + 4) {
        var scheduletime1 = schedule_time[i];
        var scheduletime2 = schedule_time[i + 1];
...
// The right and left change options on the schedule window to show different timetables of the line on the stop
var dayIndex = 1;
...
var count = 0;
// The following function opens the time table when the user clicks on Open Schedule or on the line number on TIme table tab
...
// Close the timetable when the user clicks on close
$(function() {
  $("#close-tt").click(function() {
    var line_num = $("#schedule-line-num").html();
    var row_id = "#row"+line_num;
    $(row_id).click();
...
// Open the line numbers schedule on the timetable tab for stops
